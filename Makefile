...
.PHONY: install dev test test-fast test-v test-all lint format security run run-dry preview-analysis demo-analysis clean env pre-commit graph eval contract record smoke-test snapshot-update budget-report bump-patch bump-minor bump-major build publish help wt-new wt-open wt-headless wt-list wt-rm wt-rm-all web web-dev web-check web-test web-install dev-board dev-poker
