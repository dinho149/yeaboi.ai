...
.PHONY: install dev test test-fast test-v test-all lint format security run run-dry preview-analysis demo-analysis clean env pre-commit graph eval contract record smoke-test snapshot-update budget-report bump-patch bump-minor bump-major build publish help wt-new wt-open wt-headless wt-issue wt-list wt-rm wt-rm-all web web-dev web-check web-test web-install dev-board dev-poker dev-deck dev-editable site-seo site-check site-og site-serve pr-feedback cowork-setup cowork-check cowork-teardown demo demo-render cowork-agenda
...
preview-analysis: ## Page through every Analysis screen with fake data (no credentials)
	$(UV) run python scripts/preview_analysis.py $(ARGS)

demo-analysis: ## Walk the real Analysis flow in the TUI with dummy data (no credentials)
	$(UV) run python scripts/demo_analysis.py $(ARGS)
