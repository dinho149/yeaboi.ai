...
var _heroDuckTimer = null;
var _heroStepIO = null;
var _heroSizeHandler = null;


// The click-the-duck gag (render_head_shades + SHADES_LIFT_SEQUENCE): the shades
// float up off the crown, revealing an identical pair on the eyes underneath.
// Two chars per cell — top-half palette index, bottom-half index, '.' for a
// transparent half — because _pack_cells packs two source pixels into one cell
// and keeping only one of them is what turns the sprite into specks. Rows are
// '|'-separated. Generated by scripts/gen_landing_art.py; do not hand-edit.
var DUCK_PALETTE = ['rgb(9,14,18)','rgb(26,32,40)','rgb(34,158,122)','rgb(22,110,92)','rgb(232,240,238)','rgb(150,190,190)','rgb(96,140,144)','rgb(60,100,108)','rgb(250,176,44)','rgb(228,104,22)'];
var DUCK_FRAMES = {0:'................................|................................|...........202020202.2..........|.......12222222222222212.1......|..000040040000400400030303......|..0.0000080800000002222233......|00088888888989292222233330......|....0.0.0.0.03333333330.........|..........222222222223.3........',1:'................................|................................|...........202020202.2..........|...0.0.42020202420202010.0......|..000000400000004000030303......|..0.0000080800000002222233......|00088888888989292222233330......|....0.0.0.0.03333333330.........|..........222222222223.3........',3:'................................|................................|...0.0.4.0.000040000.0.0.0......|..0000004000000040002212.1......|..000040040000400400030303......|..0.0000080800000002222233......|00088888888989292222233330......|....0.0.0.0.03333333330.........|..........222222222223.3........',5:'................................|...0.0.4.0.0.0.4.0.0.0.0.0......|..000000400000004000.2..........|.......12222222222222212.1......|..000040040000400400030303......|..0.0000080800000002222233......|00088888888989292222233330......|....0.0.0.0.03333333330.........|..........222222222223.3........'};
var DUCK_LIFTS = [1,3,5,5,5,5,5,3,1,0];
...
function initHeroDemo() {
  ...
  if (_heroDuckTimer) { clearInterval(_heroDuckTimer); clearTimeout(_heroDuckTimer); _heroDuckTimer = null; }
  ...
  function sizePhoneTitles() {
    ...
    modes.forEach(function (m) {
      var el = m.querySelector('.tui-mode-title');
      if (!phone) { el.style.fontSize = ''; return; }
      // --cols, not textContent: the title is a mask over an empty element now,
      // so there is no text to measure the column count from.
      var cols = parseInt(el.style.getPropertyValue('--cols'), 10) || 30;
  ...
  modes.forEach(function (m) {
    m.addEventListener('click', function () {
      var i = parseInt(m.getAttribute('data-mode'), 10) || 0;
      // Scroll the matching step to the middle rather than only selecting.
      // The rail is what drives selection while scrolling, so a click that
      // changed the terminal but left the page where it was would be undone by
      // the next wheel notch — and the caption beside it would still be
      // describing a different mode. Moving the page keeps the two in step,
      // and the observer below does the selecting for us.
      if (scrollDriven && steps[i]) {
        steps[i].scrollIntoView({ block: 'center', behavior: reducedMotion ? 'auto' : 'smooth' });
        return;
      }
      show(i);
    });
  ...
  // ---- the click-the-duck gag (render_head_shades) ----
  var duckEl = document.getElementById('tui-duck');
  function paintDuck(lift) {
    var frame = DUCK_FRAMES[lift];
    if (!frame || !duckEl) return;
    var html = '';
    frame.split('|').forEach(function (row) {
      html += '<span class="r">';
      for (var i = 0; i < row.length; i += 2) {
        var t = row.charAt(i), b = row.charAt(i + 1);
        if (t === '.' && b === '.') { html += '<i></i>'; }
        else if (t === b) { html += '<i style="background:' + DUCK_PALETTE[+t] + '"></i>'; }
        else {
          html += '<i style="background:linear-gradient(to bottom,' +
            (t === '.' ? 'transparent' : DUCK_PALETTE[+t]) + ' 50%,' +
            (b === '.' ? 'transparent' : DUCK_PALETTE[+b]) + ' 50%)"></i>';
        }
      }
      html += '</span>';
    });
    duckEl.innerHTML = html;
  }
  function liftShades() {
    if (_heroDuckTimer || !duckEl) return;
    if (reducedMotion) {  // the gag, without the flutter: up, hold, down
      paintDuck(5);
      _heroDuckTimer = setTimeout(function () { paintDuck(0); _heroDuckTimer = null; }, 900);
      return;
    }
    var i = 0;
    _heroDuckTimer = setInterval(function () {
      paintDuck(DUCK_LIFTS[i]);
      if (++i >= DUCK_LIFTS.length) { clearInterval(_heroDuckTimer); _heroDuckTimer = null; }
    }, 90);
  }
  if (duckEl) {
    duckEl.addEventListener('click', liftShades);
    duckEl.addEventListener('keydown', function (e) {
      if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); liftShades(); }
    });
  }
