...
- `YEABOI_GO` — `1`/`true`: opt into the Go core sidecar for agentwatch (the strangler-fig pilot; see `contracts/v1/rpc.md` and `src/yeaboi/gocore/`). Any sidecar failure silently falls back to the Python engines
- `YEABOI_CORE_BIN` — explicit path to the `yeaboi-core` binary (`make go-build` puts it at `bin/yeaboi-core`); otherwise the `yeaboi[core]` wheel or `$PATH` is searched
