...
- `YEABOI_GO` — three-state switch for the Go core sidecar serving agentwatch (see `contracts/v1/rpc.md` and `src/yeaboi/gocore/`). Unset = **auto**: use the sidecar iff a binary is discovered (the default since the `yeaboi[core]` wheel). `0`/`false` = off; `1`/`true` = forced on (a missing binary then logs a line). Any sidecar failure silently falls back to the Python engines
