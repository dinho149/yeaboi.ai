...
|---|---|
| `src/yeaboi/web/static/**` | Never hand-resolve, never a `union` merge driver — it produces silently corrupt JS. `git checkout --theirs -- src/yeaboi/web/static && make web && git add src/yeaboi/web/static` (CLAUDE.md, Front End) |
| `frontend/src/test/fixtures/**` | Take theirs, then `uv run pytest tests/unit/test_web_wire_shapes.py` regenerates them; commit what it wrote |
| `tests/unit/__snapshots__/*.ambr` | Take theirs, then `make snapshot-update` |
| `pyproject.toml` version line, `src/yeaboi/changelog_data.json` | Take **theirs** and drop your bump entirely. `auto-version.yml` re-bumps on the PR branch, so your side is redundant; and the changelog is prepend-only, which makes every pair of release-worthy PRs collide here by construction |
| `uv.lock` | Take theirs, then `uv lock`. Note the root `version` line tracks `pyproject.toml`, and `auto-version.yml` does *not* update it — so a disagreement here is expected, not a mistake to preserve |
