...
| analysis core | `analysis/{aggregate,code_health,coverage,practices}.py` + `ai_usage.py`'s classifier block (the marker tables, `_classify_ai_*`, `aggregate_ai_markers`, `_activity_bucket`, `_collect_samples`) + `doc_quality.py`'s scoring pieces (`_AI_DISCLOSURE_CONTEXT`, `_MIN_DOC_SAMPLE`/`doc_small_sample`, `_has_ai_disclosure`, `_CLEAR_MIN`/`_UNCLEAR_MAX`, `_count_syllables`, `_clarity_metrics`, `_usefulness_metrics`, `_analyse_page_asset`, `_aggregate_doc_assets`, `_doc_findings`, `_prioritize_doc_actions`, `_fallback_doc_quality_insights` — the read/cache plumbing around them is NOT mirrored; **frozen since W7: Go is the reference for the doc-quality scoring pieces, and the frozen Python twin (`tests/unit/test_migration_freeze.py`) changes only for user-facing bugfixes, mirrored from Go**) + `tools/team_learning.py`'s `_insight_item`/`_INSIGHT_MAX_ITEMS` | `go/internal/analysis/` |
| exports | `retro/export.py`'s pure builders (`_title`, `_reactions_str`, `build_retro_markdown`, `_card_payload`, `retro_export_args`) + `poker/export.py`'s (`_title`, `_pts`, `_votes_str`, `build_poker_markdown`, `_ticket_payload`, `poker_export_args`) + `artifacts/render.py` (all of it) + `html_theme.py`'s `safe_url`/`history_series`/`trend` + `markdown_convert.md_table_cell` + `artifacts/paths.escape_value` + `retro/board.py`'s grid/status constants + `RetroReport.by_grid` + the stores' `report_from_dict` — **load-bearing beyond retro/poker**: `html_theme.py` and `render.py` are shared with five other exporters, so a change there now needs the Go twin even from another mode's work; **frozen since W8: Go is the reference for the export builders, and the frozen Python twins (`retro/export.py`, `poker/export.py`, `artifacts/render.py` — `tests/unit/test_migration_freeze.py`) change only for user-facing bugfixes, mirrored from Go** | `go/internal/exports/` |
| foundations | `paths.py` + `config.py` (every getter/setter) + `logging_setup.py` + `redaction.py` + `cli.py`'s `build_parser()` tree (flags, subcommands, help bytes) + the bundled-changelog parsing — mirrored behind the **CLI golden contract** (`contracts/cli/README.md`), not RPC: a behaviour change here regenerates the `tests/parity/foundations` goldens (`uv run python -m tests.parity.foundations.regen`) *after* the Go twin mirrors it, or `make go-test`/`make parity` fail on the same files | `go/internal/{home,dotenv,config,logfile,changelog}` + `go/cmd/yeaboi` (parse tree; hidden, unshipped until W19) with `go/internal/{argparse,argview,foundations}` |
