...
**The fleet does exactly three things: it maintains what exists, it builds one provider integration at a time, and it rewrites the codebase in Go one wave at a time — and one workstream watches it do all of it.** Seventeen standing workstreams. Fourteen are *maintenance* — one per mode: **planning** (`agent/` + `prompts/` + `ui/session/`), standup, analysis, reporting, poker, retro, performance, roadmap, artifacts-sharing and **agents** (the agentwatch family + `pricing.py`), plus the cross-cutting tui-ux (including the `usage` and `settings` pages), web-ux, platform and security — and they may only return `bug`, `chore`, `docs` or `security`. **Thirteen of the fourteen run a sweep** (`cowork/routines/cron/*-sweep.md`, which is what `sweep-procedure.md` means by "all thirteen"); `security` is one of them and sweeps twice weekly. **`agents` is the exception and has no sweep at all** — its daily routine posts the agent standup digest instead, so it is the one *maintenance* charter whose paths no scout opens. That is a stated choice in `workstreams/agents.md` ("no sweep yet — the family is new"), not an oversight. (The other three charters run campaigns or `retune.md` rather than sweeps, and a scout is spawned only by a sweep.) **`integrations` is the one that builds features**, weekdays, as a week-long campaign. **`go-migration` is the second builder**, weekdays, executing the 13-wave Go rewrite program (`cowork/migration/program.md`) unattended behind the byte-parity gate — the merged program doc is the standing approval, and the weekly 🐹 progress post plus a 🌊 post per merged wave carry its story (`house-rules.md`, **The migration lane**). Sweep cadence is tiered to surface size: weekly for large, fortnightly for mid, monthly for small. Routines are **account-scoped, not repo files**; `cowork/` is the versioned source of truth their prompts point at. Start at `cowork/README.md`.
...
- **One workstream's subject is the fleet itself.** `cowork/workstreams/fleet.md` owns `cowork/`
  (**except the constitution, and except `cowork/migration/`, which is go-migration's**),
  `scripts/cowork_*.py`, `scripts/hygiene_lens.py` and
...
- **A sweep now looks for things, not just at things.** `cowork/hygiene-lenses.md` holds the standing
  ...
  `.github/hygiene/lens-policy.yml` with a `why` per entry. **Six lenses, and they are not all on the
  same sweeps**: `dead-code`, `assertion-free-tests` and `layering` run on all thirteen; `duplication`
  on five; `crash-fuzz` on two (tui-ux and planning); `stale-flags` on one. The spread is per-lens and
  lives in `cowork/hygiene-lenses.md` and `lens-policy.yml` — a lens that finds nothing useful on a
  surface is withheld from it rather than tuned, because an answered question re-filed weekly under a
  new name is how a lens stops being read. Retro returns nothing on every lens it runs, which is the
  only evidence anyone has that these are detectors rather than work generators.
...
- **The fleet bookends the day: what is about to run, then what ran.** `cron/day-ahead.md` posts the schedule at 05:45 UTC, before the earliest sweep, and `cron/shipped-standup.md` posts at 18:00 UTC — **one message per area that moved**, each carrying that workstream's merged PRs with the trace behind each (what proved it, the check result, the review verdict, the merge time), what is still building, what is stuck, and the pre-release it all landed in. It was one roll-up grouped by *type* until 2026-08-16, which meant a fix in `analysis/` arrived as a `[bug]` line between a go-migration wave and a platform chore, and the twelve maintenance workstreams that post nothing in the channel had no voice at all; `scripts/cowork_evening.py` renders the messages and the routine posts them verbatim, because a model deciding *how many* messages and *which* is one that can post an area twice or drop one silently. The one section that could not be sliced by area went with it: 🩺 **Fleet health** is its own message, since a routine that never ran has no area and a thread reply does not notify. The evening post also replaced the per-PR ship note that `pr-merged-close-loop.md` used to fire once per merge with batching explicitly forbidden. `day-ahead` was itself deleted once, on the reasoning that announcing what is *about* to run is worth less than reporting what ran; it came back because the question it answers is not "what will the fleet do" but **when to expect something in Slack**, which nothing else says. It is one of three routines exempt from "nothing to do is a valid outcome, exit quietly" — the others being the 🧭 agent digest and the weekly 🐹 progress bar — because all three report a *standing* thing rather than a findings run: silence from a findings routine means it found nothing, while silence from a schedule, a daily digest or a progress bar is ambiguous. It composes nothing: `scripts/cowork_setup.py --agenda` matches every cron against the date, converts to `DISPLAY_TZ` (UTC stays the source of truth and rides along in brackets), and the routine posts the `lines` it is handed; `make cowork-agenda` / `/cowork today` print the same string. Nothing is held back from it — the evening post is named too, because a schedule that omits half of what you are waiting for is not one. Every routine file still carries a `**Summary**` line capped at 90 characters.
