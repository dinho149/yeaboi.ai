...
* :mod:`~yeaboi.artifacts.render` — drawing reader-added notes and fields
* :mod:`~yeaboi.artifacts.store` — the append-only log those corrections live in

Nothing here touches HTTP or the browser. Serving lives in :mod:`yeaboi.sharing`,
and it is a deliberate split: an edit that cannot be applied is a validation
question, not a request question, and it should be answerable in a unit test
...
from yeaboi.artifacts.store import ArtifactEditStore, artifact_ref, base_hash
