...
class _PokerHandler(BaseHTTPRequestHandler):
    ...
    def _authed(self) -> bool:
        ...
        token is not consulted, so a leaked link is not a way in. The host's own
        loopback requests stay token-gated, because cloudflared connects from
        ``127.0.0.1`` and requiring a JWT everywhere would lock the host out of
        their own board.
        """
