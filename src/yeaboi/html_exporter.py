...
def plan_export_args(graph_state: dict, stage: str = "complete") -> dict[str, object]:
    """Build the chrome + payload arguments for a sprint plan.
    ...
    Split from :func:`build_export_html` so the payload can be rebuilt without a
    page around it; see :func:`yeaboi.standup.export.standup_export_args`.
    """
...
def build_export_html(graph_state: dict, stage: str = "complete", *, document_title: str = "") -> str:
    """Build a self-contained HTML report from available graph state artifacts."""
    from yeaboi.html_theme import export_page

    return export_page(**plan_export_args(graph_state, stage), document_title=document_title)  # type: ignore[arg-type]
