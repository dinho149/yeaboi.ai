...
def _edits(anchor: str, artifact, fields: Sequence[str]) -> dict:
    """The ``{field: {path, value}}`` map that makes a region correctable.

    Emitted only for a document that is actually being served editable, and
    absent from every file export — so a downloaded report is byte-for-byte what
    it always was, and the ten committed wire fixtures do not move.

    The **raw artifact value** rides along beside the path, and that is the whole
    point. This payload is a one-way projection: ``_team_summary_runs`` shreds
    prose into sentences of link-runs with no inverse, so an editor opened on
    what is drawn could never hand back something the server can store. It opens
    on the string instead, and the server re-derives the drawing.
    """
    out: dict = {}
    for field in fields:
...
def _member_payload(
    ...
    *,
    ...
    if editable:
        ...
        anchor = f"member_updates[name={escape_value(m.name)}]"
        out["anchor"] = anchor
        out["edit"] = _edits(
            anchor,
            m,
...
def standup_export_args(
    ...
    args = dict(
        ...
        report={
            ...
            **({"edit": _edits("", report, ("team_summary", "confidence_rationale"))} if editable else {}),
        },
