...
The HTML uses the shared design system (``html_theme``) so standup pages look
consistent with the rest of the app, with semantic tokens for the confidence
badge and a shared notices panel.
...
def build_standup_html(report: StandupReport) -> str:
    ...
    parts.append(notice_block("Notices", report.warnings or []))
