...
def _item_payload(item, *, editable: bool = False) -> dict:
    """One delivered ticket as data."""
    out = {"key": item.key, "title": item.title, "status": item.status}
    if item.assignee:
        out["assignee"] = item.assignee
    if editable:
        # Only the title. `key` is the natural key this row is addressed by, and
        # a key you can edit is not a key; `status` comes from the tracker.
        anchor = row_anchor("delivered_items", "key", item.key)
        out["anchor"] = anchor
        out["edit"] = edit_map(anchor, item, ("title",))
    return out


def reporting_export_args(
    report: DeliveryReport, *, history: Sequence[dict] = (), editable: bool = False
) -> dict[str, object]:
    ...
    args = dict(
        ...
        report={
            ...
            "items": [_item_payload(it, editable=editable) for it in report.delivered_items],
