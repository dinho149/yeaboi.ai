...
def deck_payload(report: DeliveryReport, *, theme: str = "midnight", style: DeckStyle | None = None) -> dict:
    ...
    * **The palettes** are shipped whole rather than as a theme name. There is
    ...
    return {
        # The same chrome every other surface draws its furniture from. The deck
        # uses only the footer — a slide deck's title slide already is a
        # masthead, and a terminal frame on a projector would be absurd — but it
        # takes the credit from here rather than hardcoding it in TSX, which is
        # what made it the one "Generated by yeaboi" of ten that could drift.
        "chrome": build_chrome(
            mode="reporting",
            title=project,
            wordmark="report",
            subtitle=_period_line(report),
        ),
        "project": project,
...
def build_presentation_html(report: DeliveryReport, *, theme: str = "midnight", style: DeckStyle | None = None) -> str:
    ...
    html = render_page(
        ...
        # chosen palette — and, now that a deck palette contributes an accent
        # rather than a whole surface, what it keeps showing underneath.
        #
        # `data-deck-theme` is no longer written here. It was a second theme
        # attribute for a second theme system, and there is only one now: the
        # visitor's `data-theme` owns the surface, a deck palette adds
        # `--deck-accent` on top. `palette.ts` still records the palette's name
        # on the element at runtime, which is all anything ever read it for.
        html_attrs='data-mode="reporting"',
        body=_NOSCRIPT,
    )
    logger.info(
