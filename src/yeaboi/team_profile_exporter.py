...
def profile_export_args(
    ...
    *,
    ...
    """Build the chrome + payload arguments for a team-profile report.

    The content travels as a ``profile`` payload of generated :ref:`blocks
    <Block>` and is drawn in the browser. Nothing here builds markup, which is
    why this function lost roughly a thousand lines and all of its escaping.
    ...
    Split from :func:`build_team_profile_html` so an editable shared profile can
    rebuild the payload without a page around it.
    """
    ex = examples or {}
    sections: list[dict] = []
    ...
    return with_annotations(args, profile)
...
def build_team_profile_markdown(
    ...
    *,
    ...
    lines.extend(annotations_markdown(profile.annotations))
