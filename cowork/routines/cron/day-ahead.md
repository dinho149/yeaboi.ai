...
retrospective or decision-seeking: the digest carries proposals two and a half hours from now,
and `shipped-standup` carries what already merged, twelve hours later. Nobody was saying what
was *about* to run, and the schedule itself only existed as twenty-one cron expressions in a
markdown table — `30 7 11,25 * *` is written for a scheduler, not for somebody deciding
whether to look at Slack this morning.
...
others being `agents-standup.md`'s daily digest and `go-migration-progress.md`'s weekly bar — and
