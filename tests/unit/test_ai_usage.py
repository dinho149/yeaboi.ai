...
class TestClassifyAiMarkers:
    ...
    def test_codex_attribution_forms(self):
        assert _classify_ai_markers("fix\n\nCo-authored-by: Codex <bot@openai.com>") == {"codex"}
        assert _classify_ai_markers("Summary\n\nhttps://chatgpt.com/codex/tasks/task_e_123") == {"codex"}
        assert _classify_ai_markers("docs\n\nGenerated by Codex") == {"codex"}

    def test_codex_prose_mention_is_not_ai_usage(self):
        assert _classify_ai_markers("we evaluated codex for the team") == set()


class TestClassifyAiAuthors:
    def test_agent_bot_accounts_match(self):
        assert _classify_ai_authors("devin-ai-integration[bot]") == {"devin"}
        assert _classify_ai_authors("copilot-swe-agent[bot]") == {"copilot"}
        assert _classify_ai_authors("Copilot") == {"copilot"}
        assert _classify_ai_authors("openai-codex[bot]") == {"codex"}
        assert _classify_ai_authors("chatgpt-codex-connector[bot]") == {"codex"}
        assert _classify_ai_authors("Cursor Agent") == {"cursor"}
        assert _classify_ai_authors("google-labs-jules[bot]") == {"gemini"}

    def test_agent_emails_match(self):
        assert _classify_ai_authors("Claude", "noreply@anthropic.com") == {"claude"}
        assert _classify_ai_authors("someone", "cursoragent@cursor.com") == {"cursor"}
        assert _classify_ai_authors("bot", "devin-ai-integration@devin.ai") == {"devin"}

    def test_aider_author_suffix(self):
        assert _classify_ai_authors("Ava Lee (aider)") == {"aider"}

    def test_human_names_never_match(self):
        assert _classify_ai_authors("Devin Smith", "devin.smith@example.com") == set()
        assert _classify_ai_authors("Claudette Marchand", "claudette@example.com") == set()
        assert _classify_ai_authors("Ava", "ava@example.com") == set()

    def test_dependency_bots_are_not_ai(self):
        assert _classify_ai_authors("dependabot[bot]") == set()
        assert _classify_ai_authors("renovate[bot]") == set()

    def test_unknown_ai_looking_bot_is_other_ai(self):
        assert _classify_ai_authors("some-ai-agent[bot]") == {"other_ai"}

    def test_empty(self):
        assert _classify_ai_authors("", "") == set()


class TestClassifyAiItem:
    def test_branch_prefixes_mark_agent_prs(self):
        assert _classify_ai_item({"kind": "pr", "title": "Fix login", "branch": "codex/fix-login"}) == {"codex"}
        assert _classify_ai_item({"kind": "pr", "title": "Add tests", "branch": "copilot/add-tests"}) == {"copilot"}

    def test_human_branches_do_not_match(self):
        assert _classify_ai_item({"kind": "pr", "title": "Docs", "branch": "feature/codex-docs"}) == set()
        assert _classify_ai_item({"kind": "pr", "title": "Style", "branch": "fix-cursor-styles"}) == set()

    def test_author_and_text_union_suppresses_other_ai(self):
        item = {
            "kind": "commit",
        ...
        assert _classify_ai_item(item) == {"claude", "devin"}
...
class TestAggregate:
    ...
    def test_bot_authored_commit_with_plain_message_counts(self):
        # Agents that commit under their own account often leave no trailer.
        sig = aggregate_ai_markers(
            [{"kind": "commit", "author": "devin-ai-integration[bot]", "title": "fix flaky test", "body": ""}]
        )
        assert sig.ai_commits == 1
...
class TestRunAiAdoption:
    ...
    def test_member_filter_retains_agent_authored_items(self, monkeypatch):
        items = (
            [
                {"kind": "commit", "author": "Alice", "title": "x", "body": "", "source": "github"},
                {"kind": "commit", "author": "Bob", "title": "y", "body": "", "source": "github"},
                {
                    "kind": "commit",
                    "author": "devin-ai-integration[bot]",
                    "title": "agent fix",
        ...
        # Alice's commit + the agent's commit are kept; Bob's human commit drops.
        assert sig.scanned_commits == 2
        assert sig.ai_commits == 1
        assert sig.per_tool == (("devin", 1),)
