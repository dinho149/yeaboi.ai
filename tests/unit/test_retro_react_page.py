"""Contract tests for the React retro board page.
...
from yeaboi.retro.page import board_config, build_board_html, build_react_board_html
from yeaboi.retro.server import RetroServer


@pytest.fixture
def page() -> str:
    return build_react_board_html("Sprint 42")
...
class TestBootIsland:
    ...
    def test_island_is_script_safe(self):
        ...
        """
        html = build_react_board_html("</script><img src=x onerror=alert(1)>")
...
class TestUiFlag:
    def test_defaults_to_the_legacy_page(self, monkeypatch: pytest.MonkeyPatch):
        monkeypatch.delenv("RETRO_UI", raising=False)
        # The legacy page's marker: a hand-written inline script, not a bundle.
        assert "let TOKEN = new URLSearchParams" in build_board_html()

    def test_react_flag_selects_the_bundle(self, monkeypatch: pytest.MonkeyPatch):
        monkeypatch.setenv("RETRO_UI", "react")
        html = build_board_html("Sprint 42")
    ...
    def test_unknown_value_keeps_the_legacy_page(self, monkeypatch: pytest.MonkeyPatch):
        # An upgrade must land on exactly what the install already had, so
        # anything that is not the opt-in word falls back rather than guessing.
        monkeypatch.setenv("RETRO_UI", "preact")
        assert "let TOKEN = new URLSearchParams" in build_board_html()
...
class TestServedPageIsTokenFree:
    ...
    def test_served_page_never_contains_the_token(self, monkeypatch: pytest.MonkeyPatch, ui: str):
        ...
        monkeypatch.setenv("RETRO_UI", ui)
