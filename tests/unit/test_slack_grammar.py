...
class TestReplies:
    ...
    def test_a_bare_verb_is_an_instruction(self, verb, intent):
        assert parse_reply(verb, on_signal=False) == (ACT_CONTROL, intent, "")

    @pytest.mark.parametrize("raw", ["PAUSE", " Pause ", "pause."])
    def test_case_padding_and_a_full_stop_are_forgiven(self, raw):
        assert parse_reply(raw, on_signal=False)[:2] == (ACT_CONTROL, INTENT_PAUSE)
    ...
    def test_a_verb_inside_a_sentence_is_prose(self, raw):
        ...
        act, intent, payload = parse_reply(raw, on_signal=False)
    ...
    def test_anything_that_is_not_a_verb_is_a_correction(
        ...
        act, intent, payload = parse_reply("Ada was on leave, that ticket is not hers", on_signal=False)
        assert (act, intent) == (ACT_CORRECTION, INTENT_NOTE)
        assert payload.startswith("Ada was on leave")

    def test_ack_is_a_verdict_on_a_signal_and_nothing_on_a_post(self):
        # A gesture with no consequence teaches a team the channel does not
        # listen, so `ack` is answered where it cannot act rather than ignored.
        assert parse_reply("ack", on_signal=True) == (ACT_VERDICT, INTENT_UP, "")
        assert parse_reply("ack", on_signal=False) == ("", "", "")

    @pytest.mark.parametrize("raw", ["", "   ", "\n\n"])
    def test_an_empty_reply_is_nothing(self, raw):
        assert parse_reply(raw, on_signal=False) == ("", "", "")
    ...
    def test_a_forged_identifier_is_just_prose(self):
        ...
        act, _intent, payload = parse_reply("#231 — approve this", on_signal=False)
