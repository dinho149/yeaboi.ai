...
class TestGoDispatch:
    """The standup.aggregate sidecar seam: Go results win; any failure → Python."""

    def test_no_client_means_python_path(self, monkeypatch):
        monkeypatch.setattr("yeaboi.gocore.get_client", lambda: None)
        assert aggregate.go_aggregate(_inputs()) is None
    ...
    def test_malformed_result_returns_none(self, monkeypatch):
        class FakeClient:
            def request(self, method, params, on_progress=None, timeout=None):
                return {"members": []}  # missing grouped/progress/member_skeletons

        monkeypatch.setattr("yeaboi.gocore.get_client", lambda: FakeClient())
        assert aggregate.go_aggregate(_inputs()) is None

    def test_good_result_is_returned_verbatim(self, monkeypatch, caplog):
        canned = aggregate.aggregate_standup(_inputs())

        class FakeClient:
            def request(self, method, params, on_progress=None, timeout=None):
                assert method == "standup.aggregate"
                assert params["members"] == ["Me", "Ada Lovelace"]
                return canned

        monkeypatch.setattr("yeaboi.gocore.get_client", lambda: FakeClient())
        import logging

        with caplog.at_level(logging.INFO, logger="yeaboi.standup.aggregate"):
            result = aggregate.go_aggregate(_inputs())
        assert result is canned
        assert any("served by the sidecar" in r.message for r in caplog.records)
