...
``contracts/web/fixtures/``. ``wire.ts`` then asserts each one ``satisfies``
...
def _export_snapshots() -> dict[str, dict]:
    ...
    **Pinned against the environment.** The standup exporter linkifies bare
    ...
    prep = OneOnOnePrep(
        ...
        # improvements is deliberately left empty: with a section_state below it
        # exercises the branch where a section rides *because* it can say why it
        # is empty, which is the whole point of shipping empty sections at all.
        ...
        # One row per coverage state, so the fixture exercises every branch the
        # component has to draw rather than only the happy one.
        evidence_coverage=(
            ("tickets", "covered", "7 of 7 named this engineer."),
            ("retro", "partial", "3 retro run(s) in the period, none of which named this engineer."),
        ...
        # A ratio, a bare count with a unit, and one carrying a hint.
        metrics=(
            PerfMetric(
        ...
        # A row with a URL and a folded child, and one whose URL the allowlist
        # rejected — the row survives either way.
        evidence_items=(
            EvidenceGroup(
                source="code",
                label="Code activity",
                note="capped at 2 of 41",
                items=(
                    ActivityEvidence(
                        kind="pr",
                        key="#91",
        ...
        section_states=(("improvements", "not_configured", "No saved team analysis covers this engineer."),),
    )
...
_RANDOM_ID = re.compile(r"[0-9a-f]{12,}")
"""A ``uuid4().hex[:12]`` card id — what retro and the editable share mint."""
    ...
    def walk(node: object) -> object:
        ...
        if isinstance(node, dict):
            ...
            for key, value in node.items():
                ...
                elif key == "id" and isinstance(value, str) and _RANDOM_ID.fullmatch(value):
                    # Random ids only — a retro card's is `uuid4().hex[:12]`.
                    # Deterministic ones (a section slug, a metric key) are left
                    # alone: pinning those would hide the very thing the fixture
                    # is for, which is that a nav entry names a section that
                    # exists. `reaction_events[].id` is an int and a monotonic
                    # counter, so it is already stable.
                    counter += 1
...
class TestWireFixtures:
    def test_committed_fixtures_match_the_live_snapshots(self):
        ...
        assert not stale, (
            ...
            "commit contracts/web/fixtures/ and check types/board.ts still describes it."
        )
