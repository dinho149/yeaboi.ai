...
| GET | `/api/system/check` | `{summary, checks: [{key, label, status, detail, hint, feature}]}` — the optional-feature doctor. `status` ∈ `ok\|missing\|unsupported\|unknown`. **Offline by policy**: every probe is a filesystem/PATH/config read or a loopback-only socket — opening the page causes no egress and never triggers the cloudflared download, so a GET per open is safe |
